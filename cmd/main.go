...
func resolveClusterTLSProfile(ctx context.Context, cli client.Client, openshift, disabled bool, log logr.Logger) (configv1.TLSProfileSpec, configv1.TLSAdherencePolicy, error) {
	...
	if err != nil {
		if apiErrors.IsNotFound(err) || apimeta.IsNoMatchError(err) {
			// The APIServer config resource (or the TLSAdherence feature) is absent.
			...
			// An unexpected error (transient API failure, RBAC, timeout) must not be
			// silently downgraded to NoOpinion: on a cluster that mandates
			// StrictAllComponents that would relax the floor and let a non-compliant
			// component roll out. Fail closed, as the profile fetch above does.
